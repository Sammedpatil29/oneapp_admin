...
export class SidebarSettingsComponent {
  ...
  saveItem() {
    if (!this.currentItem.title || !this.routerLinkString) return;

    this.currentItem.routerLink = [this.routerLinkString];
    
    if (this.exactMatchCheck) {
      this.currentItem.routerLinkActiveOptions = { exact: true };
    } else {
      delete this.currentItem.routerLinkActiveOptions;
    }

    if (this.isEditing && this.editIndex > -1) {
      this.sidebarItems[this.editIndex] = { ...this.currentItem };
    } else {
      this.sidebarItems.push({ ...this.currentItem });
    }
