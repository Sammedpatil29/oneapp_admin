...
export class SidebarSettingsComponent implements OnInit {
  ...
  triggerMidUpdate() {
    this.sidebarMidService.setValue(new Date().getTime());
  }

  toggleItem(id: number) {
    let params = {
      "is_active": !this.sidebarItems.find(item => item.id === id)?.is_active
    }
    this.isUpdating = true;
    this.commonService.updateSidebarItem(id, params).subscribe((res:any)=>{
      this.sidebarItems.find(item => item.id === id).is_active = params.is_active;
      this.isUpdating = false;
      this.triggerMidUpdate();
    }, error => {
      this.isUpdating = false;
    })
  }
  ...
  deleteItem(id: number) {
    ...
    data: {
      ...
      body: `Are you sure to delete sidebar item #${id}`,
      type: 'warning'
    }
  ...
  this.commonService.deleteSidebarItem(id).subscribe((res:any)=>{
    this.dialog.open(AlertdialogComponent, {
      data: {
        title: 'success',
        body: 'Sidebar item deleted successfully',
        type: 'success',
      },
    });
    this.triggerMidUpdate();
    ...
    this.getSidebarItems()
      ...
      data: {
        title: 'error',
        body: 'Error while deleting Sidebar Item',
        type: 'error',
      },
    });
      this.isDeleting = false
  ...
  saveItem() {
    ...
    if (this.isEditing && this.editIndex > -1) {
      this.commonService.updateSidebarItem(this.sidebarItems[this.editIndex].id, params).subscribe((res:any)=>{
        this.dialog.open(AlertdialogComponent, {
          data: {
            title: 'success',
            body: 'Sidebar item updated successfully',
            type: 'success',
          },
        });
        this.triggerMidUpdate();
        ...
        this.dialog.open(AlertdialogComponent, {
          data: {
            title: 'error',
            body: 'Error while updating Sidebar Item',
            type: 'error',
          },
        });
      })
    } else {
      this.commonService.addSidebarItem(params).subscribe((res:any)=>{
        this.dialog.open(AlertdialogComponent, {
          data: {
            title: 'success',
            body: 'Sidebar item added successfully',
            type: 'success',
          },
        });
        this.triggerMidUpdate();
        ...
        this.dialog.open(AlertdialogComponent, {
          data: {
            title: 'error',
            body: 'Error while adding Sidebar Item',
            type: 'error',
          },
        });
      })
